...
namespace WindowsExplorer
{
    ...
    public class ExplorerTree : System.Windows.Forms.UserControl
    {
        ...
        private FileSystemTreeView tvwMain;
        ...
        private void InitializeComponent()
        {
            ...
            this.tvwMain = new FileSystemTreeView();
        ...
        private void ExplorerTree_Load(object sender, System.EventArgs e)
        {
            ...
            //tvwMain.Nodes.Clear();
            //DirectoryInfo directoryInfo = new DirectoryInfo(@"D:\Media\");
            //if (directoryInfo.Exists)
            //{
            //    tvwMain.Load(directoryInfo.FullName);
            //    tvwMain.TopNode.Expand();
            //}
        ...
        private void GetFiles(TreeNode parentNode)
        {
            var pattern = "*.*";

            // Get the items from the file system, and add each of them to the ListView,
            // complete with their corresponding name and icon indices.
            string[] fileList = new DirectoryInfo(parentNode.Tag.ToString()).
                GetFilesByExtensions(
                    pattern.Equals("*.*") ? null : pattern.Replace("*", string.Empty).Split(';')
                )
                .Select(f => f.FullName).ToArray();

            /*string[] fileList = Directory.GetFiles(parentNode.Tag.ToString());*/

            Array.Sort(fileList);

            //check if dir already exists in case click same dir twice
            if (fileList.Length == parentNode.Nodes.Count)
                return;

            //add each file in selected dir
            for (int i = 0; i < fileList.Length; i++)
            {
                node = new TreeNode();
                node.Tag = fileList[i]; //store path in tag
                node.Text = fileList[i].Substring(fileList[i].LastIndexOf(@"\") + 1);
                node.ImageIndex = GetIconImageIndex(node.Tag.ToString());
                parentNode.Nodes.Add(node);
            }
        }
        ...
        public int GetIconImageIndex(string path)
        {
            ...
            return (int)_systemIcons[System.IO.Path.GetExtension(path)];
        }
        ...
        public void setCurrentPath(string strPath)
        {
            ...
            else
            {
                ...
                if (inf.Exists)
                {
                    ...
                    // Handled cross thread when using BGW
                    txtPath.Invoke(new Action(() => txtPath.Text = strPath));
                }
                else
                    //txtPath.Text = Application.StartupPath;
                    txtPath.Invoke(new Action(() => txtPath.Text = Application.StartupPath));
                ...
                foreach (string drive in drives)
                {
                    ...
                    try
                    {
                        ...
                        if (Directory.Exists(drive))
                        {
                            foreach (string dir in Directory.GetDirectories(drive))
                            {
                                //tvwMain.AddDirectoryNode(dir, nodeDrive);
                                dir2 = dir;
                                node = new TreeNode();
                                node.Tag = dir;
                                node.Text = dir.Substring(dir.LastIndexOf(@"\") + 1);
