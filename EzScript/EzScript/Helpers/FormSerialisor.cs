...
public static class FormSerialisor
{
    ...
    {
        foreach (Control childCtrl in c.Controls.Cast<Control>().OrderBy(o => o.TabIndex))
        {
            if (!(childCtrl is Label))
            {
                ...
                if (childCtrl is DateTimePicker)
                {
                ...
                {
                    // need to account for multiply selected items
                    ListView lst = (ListView)childCtrl;
                    for (int i = 0; i < lst.SelectedIndices.Count; i++)
                    {
                        xmlSerialisedForm.WriteElementString("SelectedIndex", (lst.SelectedIndices[i].ToString()));
                    }
    ...
    static Control GetImmediateChildControl(Control[] ctrl, Control currentCtrl)
    {
        Control c = null;
    ...
    public static void SetControlProperties(string XmlFileName, params Control[] ctrlsToSet)
    {
        if (File.Exists(XmlFileName))
        {
            //XmlDocument xmlSerialisedForm = new XmlDocument();
            ...
            XDocument doc = XDocument.Parse(File.ReadAllText(XmlFileName));

            foreach (var ctrlToSet in ctrlsToSet)
            {
                XElement result = doc.Descendants("Control")
       .FirstOrDefault(el => el.Attribute("Name")?.Value == ctrlToSet.Name);
                if (result != null)
                {
                    // Convert XElement to XmlNode
                    XmlDocument xD = new XmlDocument();
                    xD.LoadXml(result.ToString());
                    XmlNode n = xD.FirstChild;
                        ...
                        if (ctrlToSet.GetType().ToString() == controlType)
                        {
                            ...
                            switch (controlType)
                            {
                                ...
                                case "System.Windows.Forms.DateTimePicker":
                                    ...
                                    ((System.Windows.Forms.DateTimePicker)ctrlToSet).Value = DateTime.Parse(n["Value"].InnerText);
                                    break;
                                ...
                                case "System.Windows.Forms.ListView":
                                    // need to account for multiply selected items
                                    ListView lv = (ListView)ctrlToSet;
                                    XmlNodeList xnlSelectedIndexLV = n.SelectNodes("SelectedIndex");
                                    for (int i = 0; i < xnlSelectedIndexLV.Count; i++)
                                    {
                                        var index = Convert.ToInt32(xnlSelectedIndexLV[i].InnerText);
